...
public class SistemaController {
	...
	public List<Vehiculo> listarVehiculosDisponibles(){
		List<Vehiculo> disponibles = vehiculoService.listarVehiculosDisponibles();
		return disponibles;
	}
	...
	public void iniciarViaje(String usuarioDni, String vehiculoId) {
		...
		Vehiculo vehiculo = vehiculoService.listarVehiculosDisponibles().stream()
				.filter(v -> v.getId().equals(vehiculoId)).findFirst().orElse(null);
	...
	public void finalizarViaje(String usuarioDni, String vehiculoId) {
		Usuario usuario = usuarioService.buscarUsuarioPorDni(usuarioDni);
		Viaje viaje = usuario.getHistorialViajes().stream()
				.filter(v -> v.getVehiculo().getId().equals(vehiculoId) && v.getFin() == null).findFirst().orElse(null);
